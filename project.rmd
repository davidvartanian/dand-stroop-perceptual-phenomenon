...
tests is significant. I will use $\bar{C}$ as the sample mean of the *Congruent* 
variable, and $\bar{I}$ as the sample mean of the *Incongruent* variable. Since we
don't have population data, the following hypotheses are based only on sample
data. This means that we don't have population information and I will need to
infere it from the sample data.

$$H_0: \bar{C} = \bar{I}$$

The *null* hypothesis expects that the difference between $\bar{C}$ and $\bar{I}$
will not statistically significant.

$$H_a: \bar{C} < \bar{I}$$

On the other hand, the *alternative* hypothesis expects that $\bar{C}$ will be 
significantly less than $\bar{I}$.
...
* $\bar{I} = `r Ybar`$
* $N = `r N`$
* Point Estimate ($\bar{C} - \bar{I}$) = `r point.estimate`
* Variance = `r variance`
